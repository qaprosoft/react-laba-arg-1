...
function catMouse(map, moves) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    if (matrix[i].indexOf(CAT) !== NOT_FOUND) {
      indexCat = matrix[i].indexOf(CAT);
      places.push(indexCat);

      levelCat = i;
      levels.push(levelCat);
    } else if (matrix[i].indexOf(MOUSE) !== NOT_FOUND) {
      indexMouse = matrix[i].indexOf(MOUSE);
      places.push(indexMouse);

      levelMouse = i;
      levels.push(levelMouse);
    }
  }
